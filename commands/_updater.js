...
//---------------------------------------------------------------------------
     ...
     smd({
        ...
        async(tribalchief, msg, text,{ isCreator }) => {
                ...
                await msg.send(" *TRIBALCHIEF_MD Updater Started...!*\n\n*Please wait you have new updates*\n *───────────────────────────*\n"+update +"\n\n\n"+Config.caption);
                await sleep(3000);
          try{
...
smd({   pattern: "restart", desc: "To restart bot",category: "tools", filename: __filename }, async(tribalchief, msg,text,{ isCreator }) => {  if (!isCreator) return msg.reply(tlang().owner);  const { exec } = require("child_process"); msg.reply('Restarting'); exec('pm2 restart all'); });
