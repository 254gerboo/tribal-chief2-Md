...
*
   ...
   * @author : 254gerboo
   * @github : https://github.com/@254gerboo
...
///============================================================================================
smd({ pattern: "ptv", desc: "send ptv Message of video", category: "whatsapp",filename: __filename},async(tribalchief, msg, text,{cmdName , isCreator}) => {
   ...
   return await forwardMessage(msg.chat, tribalchief, msg, cmdName )
})////------------------------------------------------------------
smd({pattern: "#", desc: "Save whatsapp status",category: "whatsapp",filename: __filename},async(tribalchief, msg, text,{cmdName , isCreator}) => {
   if(!msg.quoted) return await msg.send("*Uhh Please, reply to whatsapp status*")
   let TRIBAL_CHIEF_ᴍᴅ_num = await tribalchief.bot.decodeJid(msg.user)
   return await forwardMessage(TRIBAL_CHIEF_ᴍᴅ_num, tribalchief, msg, cmdName )
})//--------------------------------------------------------------------
smd({pattern: "save",desc: "Save Message to log number",category: "whatsapp",filename: __filename},async(tribalchief, msg, text,{cmdName , isCreator}) => {
   if(!isCreator) return await msg.send(tlang().owner)
   if(!msg.quoted) return await msg.send("*Uhh Please, reply to a Message*")
   let TRIBAL_CHIEF_ᴍᴅ_num = await tribalchief.bot.decodeJid(msg.user)
   return await forwardMessage(TRIBAL_CHIEF_ᴍᴅ_num, tribalchief, msg, cmdName )
})///================================================================================
smd({ on: "text" }, async (tribalchief,msg,text)=> {
   if(msg.quoted && msg.text.toLowerCase().includes("send") ){
       let TRIBAL_CHIEF_ᴍᴅ_num = await tribalchief.bot.decodeJid(msg.user)
       if(msg.quoted.sender === TRIBAL_CHIEF_ᴍᴅ_num && msg.quoted.chat === 'status@broadcast' ){ return await forwardMessage(msg.chat, tribalchief.bot, msg, 'send' ); }
   }
})//==================================================================
...
async(tribalchief, msg,text,{cmdName , isCreator}) => {
   ...
   const TRIBAL_CHIEF_ᴍᴅ_num = await tribalchief.bot.decodeJid(msg.user); return await updateProfilePicture(tribalchief , TRIBAL_CHIEF_ᴍᴅ_num , msg , 'pp' );
 })
