...
smd({pattern: "tts",desc: "text to speech.",category: "downloader",filename: __filename,use: '<Hii,this is Suhail>',},
        async(tribalchief, citel, text) => {
            ...
            return tribalchief.bot.sendMessage(citel.chat,{audio: {url: ttsurl}, mimetype: "audio/mpeg", fileName: `tts Suhail Md.m4a` },{quoted: citel } );
        }
...
async(tribalchief, citel, text) => {
  ...
  var titleYt = vid[1],file = false
        try {   
            ...
            let randomName = `./temp/gerboo-Md ${vid[1]}.mp3`;
            const stream = ytdl(urlYt, {  filter: (info) => info.audioBitrate == 160 || info.audioBitrate == 128,  }).pipe(fs.createWriteStream(randomName));
          file = randomName
            await new Promise((resolve, reject) => {   stream.on("error", reject); stream.on("finish", resolve);    });
        } catch(e){
  ...
  }catch(err){ return await tribalchief.bot.sendMessage(citel.chat, {document: {url : file } ,mimetype: 'audio/mpeg',fileName: `Suhail-Md ${vid[1]}.mp3`,caption: `${Config.caption}` }) ;try{ return await fs.unlinkSync(file); } catch{}       } 
})
